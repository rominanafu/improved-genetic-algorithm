...
def localSearch_IGA(individual):
...
def globalSearch_IGA(individual):
...
def geneticAlgorithm_IGA(population, cRate, mRate, Lm, generations, dt, instance):
    ...
    for _ in range(generations):
        ...
        for i in range(populationSize):
            ...
            newIndividual = localSearch_IGA(newIndividual)
        ...
        for i in range(populationSize):
            if (lastImprov[i] > Lm):
                newIndividual = globalSearch_IGA(population[i])
                population[i] = newIndividual
                lastImprov[i] = 0
...
def localSearch_IGAm(individual):
    """ Local search strategy for expoitation """
    # Copia del ind og
    new_ind = [list(route) for route in individual]
...
def globalSearch_IGAm(individual):
    """ Global search strategy for exploration """
...
def geneticAlgorithm_IGAm(population, cRate, mRate, Lm, generations, dt, instance):
    ...
    for _ in range(generations):
        ...
        for i in range(populationSize):
            ...
            newIndividual = mutation(newIndividual, mRate)
            newIndividual = repair(newIndividual)
            newIndividual = localSearch_IGAm(newIndividual)
            newIndividual = repair(newIndividual)
        ...
        for i in range(populationSize):
            if (lastImprov[i] > Lm):
                newIndividual = globalSearch_IGAm(population[i])
                newIndividual = repair(newIndividual)
                population[i] = newIndividual
                lastImprov[i] = 0
    ...
    # Save metrics
    results.to_csv(f'results/IGAm_{instance}_{dt}.csv', index=False)

    return bestInd, bestEval

def drawSol(individual, cRate, mRate, Lm, generations, dt, instance, algo):
...
def solution_IGA(initialPopulation, dt, instance):
...
def solution(data_path, hcap, instance):
    ...
    print('Improved Genetic Algorithm (IGA)')
    solution_IGA(initialPopulation=population, dt=dt, instance=instance)
    print('Improved Genetic Algorithm Modified (IGAm)')
    solution_IGAm(initialPopulation=population, dt=dt, instance=instance)
